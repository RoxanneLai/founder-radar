...
The paid live-data gate has now exercised secure ingestion, source verification, schema repair, draft persistence, repeat-run deduplication, and this real operator preview. Fresh run `a1344244-a8ee-4361-bc79-cb0ada11b150` wrote two nonfixture drafts. The review workflow correctly blocked the event that had already started; it was archived with its evidence intact. The future September 21 listing produced a warning-bearing, approval-eligible preview and remains a draft. The two zero-source runs interrupted by the Codex network sandbox were marked cancelled without deleting their audit rows. Repeat run `100aaf50-3dd7-4ff0-ba9d-58b0a5440983` reused both source identities without duplicates and preserved their earlier valid evidence when the new relevance judgments failed validation. No event was published. An explicit decision about the remaining draft and improved relevance consistency are still pending. See [the overnight checkpoint](REVIEW-PUBLISH-PROGRESS.md) for the earlier synthetic verification.
