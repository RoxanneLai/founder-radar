...
The database foundation, ingestion implementation, and dashboard read boundary are now present. The ingestion boundary now uses OpenRouter with a configurable backend model; the original direct OpenAI adapter has been replaced. See the [ingestion guide](INGESTION.md) for current configuration. Earlier [ingestion](INGESTION-PROGRESS.md) and [dashboard](INTEGRATION-PROGRESS.md) checkpoints are historical records. Live API validation remains a separate, unverified gate. Three source-fetch replays have failed closed without event writes: one because OpenRouter omitted its fetch counter before the compatibility path existed, and two because the model returned nonconforming candidate envelopes. The extraction schema fixes candidate count to the exact selected-source count, and narrow local envelope compatibility retains strict candidate/source validation; one further replay is required.
...
A bounded Luna replay has written real drafts, but manual review caught one stale recurring-event date. The first hosted source-fetch replay returned HTTP 200 but omitted its fetch counter and was rejected; two later responses used nonconforming candidate envelopes and also failed closed. Rerun with the strict source-verdict and narrow envelope compatibility path, confirm a reported fetch count or complete bounded verdict coverage, and verify that conflicts remain blocked from review while current listings refresh. Manually review up to three current NYC drafts, rerun the same window to verify stable source identities, and inspect actual usage before expanding the search.
