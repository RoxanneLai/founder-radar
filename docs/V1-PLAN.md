...
The database foundation, ingestion implementation, and dashboard read boundary are now present. The ingestion boundary uses OpenRouter with configurable primary and schema-repair model/effort pairs; the original direct OpenAI adapter has been replaced. See the [ingestion guide](INGESTION.md) for current configuration. Earlier [ingestion](INGESTION-PROGRESS.md) and [dashboard](INTEGRATION-PROGRESS.md) checkpoints are historical records. Live discovery has produced drafts, two have been manually validated and imported, and one stale recurring event was archived with its audit evidence intact. Source-fetch responses have also exposed noncanonical candidate envelopes. Known safe variants are normalized deterministically; an optional tool-free repair request handles other source-complete schema variants under scalar-preservation checks. Qwen repair-only checks failed closed at both no and low effort. Three Muse Spark 1.3 Contributor checks were denied by OpenRouter before provider execution, including a metadata-enabled attempt that reported one available endpoint and none selected. Luna at medium effort is now the repair default. Its isolated repair check passed with two usable events and one preserved source rejection; a fresh bounded run remains the acceptance gate.
...
A bounded Luna replay wrote real drafts; manual review accepted two current listings and caught one stale recurring-event date. The accepted drafts were imported through normal validation, and the stale draft was archived without erasing provenance. The optional Luna schema repair has now passed against the preserved noncanonical response without repeating search or writing to the database. Run one fresh bounded window, confirm a reported fetch count or complete bounded verdict coverage, verify that conflicts remain blocked from review while current listings refresh, review every resulting draft against its current source, and inspect actual usage before expanding the search.
